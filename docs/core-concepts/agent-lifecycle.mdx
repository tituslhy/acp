...
- **Input Parameter:** The `inputs` parameter accepts a list of `Message` objects
...
Inside the agent function, execution happens as the agent processes inputs and yields results.
...
|--------------------------|-------------------------------|---------------------------------------------------------------------------------------------------------|
| `POST`                   | `/runs`                       | Initiates a new agent run. Requires `agent_name`, `inputs`. Optional: `session_id`, `mode` (`sync`, `async`, `stream`). Returns the initial `Run` object or stream. |
...
When using the ACP SDK, the `inputs` argument will contain the full session history from all the previous runs in the same session. `context.session_id` holds the session id, allowing agents to store additional session-related data.

```python multi_turn.py
@server.agent()
async def multi_turn(inputs: list[Message], context: Context) -> AsyncGenerator[RunYield, RunYieldResume]:
    ... # `inputs` has the full history, `context.session_id` can be used to reference extra data
```
...
This pattern allows the agent to begin processing with initial inputs, pause execution when it requires additional information, request that information, and resume from where it left off once the input is provided.
...
1. **Successful Completion.** This is the most common scenario, where the agent processes all inputs and finishes execution normally. In ACP SDK examples, this happens when the agent function yields all responses and returns control.
